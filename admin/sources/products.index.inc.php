...
if (isset($_POST['save']) && Admin::getInstance()->permissions('products', CC_PERM_EDIT)) {
    ...
    $record = $_POST;
    // CKEditor fields: keep safe formatting + <img> but strip scripts, event
    // handlers and unsafe URI schemes via an allowlist sanitiser (GHSA-43f6-gfcf-wj9c).
    $record['description'] = Sanitize::htmlClean($GLOBALS['RAW']['POST']['description']);
    $record['description_short'] = Sanitize::htmlClean($GLOBALS['RAW']['POST']['description_short']);
    $record['spec_copy'] = Sanitize::htmlClean($GLOBALS['RAW']['POST']['spec_copy']);
...
if (isset($_POST['action']) && $_POST['action']=='bulk_price' && !empty($_POST['delete']) && is_array($_POST['delete']) && Admin::getInstance()->permissions('products', CC_PERM_EDIT)) {
    $ids = array_values(array_filter(array_map('intval', $_POST['delete'])));
    if ($ids) {
        $GLOBALS['session']->set('preselected', $ids, 'bulk_price');
    }
        ...
        */
        ...
        if (($gallery = $GLOBALS['db']->select('`'.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_image_index` AS `i` INNER JOIN `'.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_filemanager` AS `f` ON i.file_id = f.file_id', false, 'i.product_id = '.$product_id, 'ORDER BY i.main_img DESC, IF(i.position = 0, 999999, i.position) ASC, i.id ASC'))) {
            ...
            foreach ($gallery as $key => $image) {
                ...
                $gallery_file_ids[] = (int)$image['file_id'];
            }
