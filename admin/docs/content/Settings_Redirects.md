...
URLs that visitors have tried to access but returned a 404 Not Found error. Shows the URI, number of hits, and when it was first logged.
...
URIs previously moved from the 404 log. These are still tracked for hit counts but hidden from the main list. Remove an ignored URI to move it back to the 404 log.
