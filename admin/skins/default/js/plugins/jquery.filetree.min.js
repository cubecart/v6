...
if (jQuery) {
    (function(a) {
                ...
                a(this).each(function() {
                        ...
                        if (a(h).children("ul").length >= 1) {
                            ...
                            a.getJSON(d.script, script_data, function(j) {
                                ...
                                a.each(j, function(p, r) {
                                    ...
                                    switch (r.type) {
                                        ...
                                        case "file":
                                            ...
                                            o.src = image_icon_path + l + "_fmcheckbox.png";
                                            a(o).attr({
                                                rel: "#" + d.name + "_" + r.id
                                            }).addClass("checkbox");
                                            if (d.unique) {
                                ...
                                if(d.group == '1' && d.root == g) {
                                    ...
                                    a(disp_tog).attr('id', 'assigned_fmtoggle');
                    ...
                    function images_toggle(disp_tog , h, show_all_images) {
                        ...
                        if(show_all_images===true) {
                            a(h).find("li.file").has("img[src$='0_fmcheckbox.png']").show();
                            a(disp_tog).text(lang.show_assigned);
                            return false;
                        } else if(show_all_images===false) {
                            a(h).find("li.file").has("img[src$='0_fmcheckbox.png']").hide();
        ...
        a("input.fmtoggle:hidden").each(function() {
            var c = (a(this).val() == "2") ? "star" : ((a(this).val() == "1") ? "1" : "0");
            var d = document.createElement("img");
            d.src = image_icon_path + c + "_fmcheckbox.png";
            if (c == "1") {
            ...
            a(c).attr({
                src: image_icon_path + d + "_fmcheckbox.png",
                alt: e,
                title: e
            })
        });
        a("#content_body").on("click", "img.imgfmtoggle", function() {
            ...
            a(e).addClass("fmtoggle").attr({
                ...
                var gallery_item = '#gallery_json li#gallery_'+d;
                switch (a(this).val()) {
                    case "1":
                        c = "1";
                        if(!$(gallery_item).exists()) {
                            var i = a(this).closest(".image").find("a").attr("href");
                            $("#gallery_json ul").append('<li id="gallery_'+d+'"><img src="'+i+'" title="'+i+'"></li>');
                        }
                        break;
                    case "2":
                        $('.fm-filelist input.fmtoggle:hidden[value="2"]').val('1');
                        var img = $("img.checkbox[src$='star_fmcheckbox.png']");
                        if(img.length>0) {
                        ...
                        if($(gallery_item).exists()) {$(gallery_item).remove();}
                        break;
                    default:
                        c = "0";
                        if($(gallery_item).exists()) {$(gallery_item).remove();}
                ...
                a(h).attr({
                    src: image_icon_path + c + "_fmcheckbox.png"
                })
            });
            var f = a(this).parent("span.action");
            a(this).before(e);
            a(this).removeClass("imgfmtoggle").addClass("fmtoggle")
        });
        a("#content_body").on("click", "img.checkbox", function() {
            ...
            switch (a(e).val()) {
                ...
                case "1":
                    if (f || !c) {
                        ...
                        a("input[value=2].fmtoggle").each(function() {
                            a(this).val("1").change()
                        });
                        var d = "2"
                    }
                ...
                default:
                    if (f) {
                        a("div.unique input.fmtoggle").val("0").change();
                        $("img.checkbox.unique[src$='1_fmcheckbox.png']").each(function() {
                            var new_src = $(this).attr('src').replace('1_fmcheckbox.png','0_fmcheckbox.png');
                            $(this).attr({src: new_src});
                        });
                        $("img.checkbox.unique[src$='star_fmcheckbox.png']").each(function() {
                            var new_src = $(this).attr('src').replace('star_fmcheckbox.png','0_fmcheckbox.png');
                            $(this).attr({src: new_src});
                        });
                        var d = "2"
                    } else if (c) {
                        var has_star = a("input.fmtoggle").filter(function(){
                            return a(this).val() === "2";
                        }).length > 0;
                        var d = has_star ? "1" : "2"
                    } else {
            ...
            if (d === "1" || d === "2") {
                a(e).data("last-changed", Date.now())
            }
            a(e).val(d).change();
            if (c && !f) {
                var selected = a("input.fmtoggle").filter(function(){
                    return a(this).val() === "1" || a(this).val() === "2";
                });
                ...
                if (selected.length > 0 && !has_star) {
                    ...
                    selected.each(function(){
                        var t = a(this).data("last-changed") || 0;
                        if (t >= latest_time) {
                            latest_time = t;
                            latest = a(this)
                        }
