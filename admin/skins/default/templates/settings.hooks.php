...
<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
    ...
    <div id="plugins" class="tab_content">
        ...
        <fieldset>
            ...
            <div>
                ...
                <img src="{$SKIN_VARS.admin_folder}/skins/{$SKIN_VARS.skin_folder}/images/plugin.png" alt=""> <a
                    href="{$plugin.edit}">{$plugin.name}</a>
            </div>
            {foreachelse}
    ...
    <div id="snippets" class="tab_content">
        ...
        <table width="70%">
            ...
            <tr>
                <td width="10"><span class="toggle"><input type="hidden" id="snippet_status_{$snippet.snippet_id}"
                                                           name="snippet_status[{$snippet.snippet_id}]"
                                                           value="{$snippet.enabled}" class="toggle"></span></td>
                <td><a href="?_g=settings&node=hooks&snippet={$snippet.snippet_id}#snippets">{$snippet.description}</a>
                </td>
                <td width="50"><span class="actions"><a
                            href="?_g=settings&node=hooks&snippet={$snippet.snippet_id}#snippets"><i
                                class="fa fa-pencil-square-o" title="{$LANG.common.edit}"></i></a> <a
                            href="?_g=settings&node=hooks&delete_snippet={$snippet.snippet_id}#snippets" class="delete"
                            title="{$LANG.notification.confirm_continue}"><i class="fa fa-trash"></i></a></span></td>
            </tr>
            {foreachelse}
            <tr>
        ...
        <fieldset>
            ...
            <div><label for="enabled">{$LANG.common.enabled}</label><span><input name="snippet[enabled]" id="enabled"
                                                                                 type="hidden" class="toggle"
                                                                                 value="{$SNIPPET.enabled}"></span>
            </div>
            <div>
                <label for="unique_id">{$LANG.hooks.unique_id}</label>
                <span>
            <input type="text" name="snippet[unique_id]" id="unique_id" class="textbox required"
                   value="{$SNIPPET.unique_id}">
            </span>
            </div>
            <div>
                <label for="priority">{$LANG.hooks.priority}</label>
                <span>
            <input type="text" name="snippet[priority]" id="priority" class="textbox required number"
            ...
            <div>
                ...
                <span>
            <input type="text" name="snippet[description]" id="description" class="textbox required"
            ...
            <input type="hidden" name="snippet[snippet_id]" id="snippet_id" class="textbox"
                   value="{$SNIPPET.snippet_id}">
            <input type="hidden" name="snippet[php_code]" id="php_code_base64" class="textbox"
                   value="{$SNIPPET.php_code_base64}">
        </fieldset>
        {/if}
    </div>
    ...
    <div id="hooks" class="tab_content">
        ...
        <fieldset>
            ...
            <a href="{$hook.edit}"><i class="fa fa-pencil-square-o" title="{$LANG.common.edit}"></i></a>
                ...
                <input type="hidden" name="status[{$hook.hook_id}]" value="{$hook.enabled}" id="status_{$hook.hook_id}"
                       class="toggle">
    ...
    <div id="hook_edit" class="tab_content">
        ...
        <fieldset>
            ...
            <div><label for="hook_name">{$LANG.hooks.name}</label><span><input type="text" name="hook[hook_name]"
                                                                               id="hook_name" class="textbox required"
                                                                               value="{$HOOK.hook_name}"></span></div>
            {if isset($PLUGINS)}
            <div>
               ...
               <select name="hook[plugin]" id="plugin" class="required">
                  <option value="">{$LANG.form.please_select}</option>
                  {foreach from=$PLUGINS item=plugin}<option value="{$plugin.plugin}"
                                                             {$plugin.selected}>{$plugin.name}</option>{/foreach}
               </select>
            </span>
            </div>
            {/if}
        ...
        <fieldset>
            ...
            <div><label for="filepath">{$LANG.hooks.path_to_file}</label><span><input type="text" name="hook[filepath]"
                                                                                      id="filepath" class="textbox"
                                                                                      value="{$HOOK.filepath}"></span>
            </div>
