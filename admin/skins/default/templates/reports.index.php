...
<form action="{$VAL_SELF}" method="post">
    <div id="results" class="tab_content">
        ...
        <table>
            ...
            <tr>{$data.value}
                <td nowrap="nowrap"><a href="?_g=orders&action=edit&order_id={$data.cart_order_id}">{$data.cart_order_id}</a>
                </td>
                ...
                <td align="left"><a href="?_g=customers&action=edit&customer_id={$data.customer_id}">{$data.first_name}
                        {$data.last_name}</a></td>
                <td>{$data.country}</td>
                <td>{$data.state}</td>
                <td>{$data.status}</td>
            ...
            {foreachelse}
            <tr>
                <td colspan="11" align="center"><strong>{$LANG.common.error_no_results}</strong></td>
            </tr>
            {/foreach}
            </tbody>
            <tfoot>
            <tr class="foot" style="font-weight: bold;">
                <td align="right">{$TALLY.orders} {if
                    $TALLY.orders==1}{$LANG.customer.order_count_single}{else}{$LANG.customer.order_count}{/if}
                </td>
        ...
        <p>
            ...
            <input type="submit" name="external_report[{$module.folder}]" class="submit"
                   value="{$LANG.customer.export_to} {$module.description}">
            {/foreach}
        </p>
    </div>
    ...
    <div id="search" class="tab_content">
        ...
        <fieldset>
            <div>
                ...
                <span>
			<input type="text" id="date_range_from" name="report[date][from]" class="textbox number date"
                   value="{$POST.date.from}"> -
			<input type="text" id="date_range_to" name="report[date][to]" class="textbox number date"
                   value="{$POST.date.to}">
		  </span>
            </div>
            <div>
