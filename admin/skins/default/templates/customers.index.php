...
<div>
  ...
  {if isset($DISPLAY_CUSTOMER_FORM)}
	...
	<div id="address" class="tab_content">
	  ...
	  <div><label for="billing">{$LANG.address.billing_address}</label><span><input type="hidden" name="address[billing]" id="billing" value="{$ADDRESS.billing}" class="toggle"></span></div>
	  <div><label for="default">{$LANG.address.default_delivery_address}</label><span><input type="hidden" name="address[default]" id="default" value="{$ADDRESS.default}" class="toggle"></span></div>
