    ...
    <form action="{$VAL_SELF}" method="post">
        <div id="groups" class="tab_content">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td align="center"><input type="hidden" name="edit_group[{$group.id}][option_required]"
                                              id="status_{$group.id}" value="{$group.required}" class="toggle"></td>
                    <td><span class="editable" name="edit_group[{$group.id}][option_name]">{$group.name}</span></td>
                    <td><span class="editable select"
                              name="edit_group[{$group.id}][option_type]">{$group.type_name}</span></td>
                    <td><span class="editable"
                              name="edit_group[{$group.id}][option_description]">{$group.description}</span>&nbsp;</td>
                    <td align="center"><a href="{$group.delete}" class="delete"
            ...
            <fieldset>
                ...
                <div><label for="new-group-name">{$LANG.catalogue.option_group_name}</label><span><input type="text"
                                                                                                         name="add-group[option_name]"
                                                                                                         id="new-group-name"
                                                                                                         class="textbox"></span>
                </div>
                <div><label for="new-group-description">{$LANG.common.description}</label><span><input type="text"
                                                                                                       name="add-group[option_description]"
                ...
                <div><label for="new-group-required">{$LANG.common.required}</label><span><input type="hidden"
                                                                                                 name="add-group[option_required]"
            ...
            <script type="text/javascript">
                {
                    if isset($OPTION_TYPE_JSON)}
                var select_data = {$OPTION_TYPE_JSON}
                {/
                    if}
        ...
        <div id="attributes" class="tab_content">
            ...
            <fieldset id="group_{$group.id}" class="field_select_target">
                ...
                <table width="100%">
                    ...
                    <tr>
                        <td width="10"><a href="#" class="handle"><img
                                    src="{$SKIN_VARS.admin_folder}/skins/{$SKIN_VARS.skin_folder}/images/updown.gif"
                                    title="{$LANG.ui.drag_reorder}"></a>
                        ...
                        <td width="10" class="actions">
                            <a href="?_g=products&node=options&delete=attribute&id={$key}" class="delete"
                               title="{$LANG.notification.confirm_delete}"><i class="fa fa-trash"
                                                                              title="{$LANG.common.delete}"></i></a>
                        </td>
                    </tr>
            ...
            <fieldset>
               ...
               <a href="#" id="group_target" class="add"><i class="fa fa-plus-circle"
                ...
                <div id="attr_source" class="inline-source">
                    <table width="100%">
                        ...
                        <tr>
                            ...
                            <td name="edit_attribute[{$key}][value_name]"><span rel="attr_name"></span><input
                                    type="hidden" rel="attr_name"></td>
                            <td width="10" class="actions">
                                <a href="#" class="remove" title="{$LANG.notification.confirm_delete}"><i
        ...
        <div id="sets" class="tab_content">
            ...
            <fieldset class="field_select_target" id="set_{$set.set_id}" rel="add_options">
                <legend>{$set.set_name} <a href="{$set.delete}" class="delete"
                                           title="{$LANG.notification.confirm_delete}"><i class="fa fa-trash"
                                                                                          title="{$LANG.common.delete}"></i></a>
                </legend>
                ...
                <div>
                  <span class="actions">
                  <a href="#" class="remove" name="member_delete" rel="{$member.set_member_id}" title=""><i
                          class="fa fa-trash" title="{$LANG.common.delete}"></i></a>
                  </span>
                    {$member.display}
                </div>
            ...
            <fieldset>
                ...
                <div><label for="new-set-name">{$LANG.common.name}</label><span><input type="text"
                                                                                       name="set_create[set_name]"
                                                                                       id="new-set-name"
                                                                                       class="textbox"></span></div>
                <div><label for="new-set-desc">{$LANG.common.description}</label><span><input type="text"
                                                                                              name="set_create[set_description]"
                                                                                              id="new-set-desc"
                                                                                              class="textbox"></span>
                </div>
            </fieldset>
