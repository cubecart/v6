...
<form id="form-settings" action="{$PHP_SELF}" method="post" enctype="multipart/form-data">
    <div id="taxclasses" class="tab_content">
        ...
        <fieldset>
            ...
            <div><input type="text" name="class[{$class.id}][tax_name]" value="{$class.tax_name}" class="textbox">
                <a href="{$VAL_SELF}&delete_class={$class.id}" class="delete" title="{$LANG.settings.tax_delete}"><i
                        class="fa fa-trash" title="{$LANG.common.delete}"></i></a> <a
                    href="?_g=settings&node=tax&assign_class={$class.id}#taxclasses" class="delete"
                    title="{$LANG.notification.confirm_update}">{$LANG.settings.assign_all_products}</a>
            </div>
            {/foreach}
        </fieldset>
        <fieldset>
            <legend>{$LANG.settings.title_tax_class_add}</legend>
            <div><label for="addclass">{$LANG.settings.tax_class_name}</label><span><input name="addclass[tax_name]"
                                                                                           id="addclass" type="text"
                                                                                           class="textbox"
                                                                                           value=""></span></div>
        </fieldset>
    </div>
    <div id="taxdetails" class="tab_content">
        ...
        <table>
            ...
            <tr>
                <td align="center"><input type="hidden" name="detail[{$detail.id}][status]" id="detail_{$detail.id}"
                                          value="{$detail.status}" class="toggle"></td>
                ...
                <td align="center"><a href="{$VAL_SELF}&delete_detail={$detail.id}" class="delete"
                                      title="{$LANG.settings.tax_delete}"><i class="fa fa-trash"
        ...
        <fieldset>
            ...
            <div><label for="detail-name">{$LANG.settings.tax_name}</label><span><input name="adddetail[name]"
                                                                                        id="detail-name" type="text"
                                                                                        class="textbox" value=""></span>
            </div>
            <div><label for="detail-display">{$LANG.settings.tax_name_display}</label><span><input
                        name="adddetail[display]" id="detail-display" type="text" class="textbox" value=""></span></div>
            <div>
    ...
    <div id="taxrules" class="tab_content">
        ...
        <fieldset>
            <table>
                ...
                <tr>
                    <td align="center"><input type="hidden" name="rule[{$rule.id}][active]" id="rule_{$rule.id}"
                                              value="{$rule.active}" class="toggle"></td>
                    <td>{$rule.class}</td>
                    <td>{$rule.detail}</td>
                    <td>{$rule.country}</td>
                    <td>{$rule.state}</td>
                    <td align="center"><input type="hidden" name="rule[{$rule.id}][goods]" id="goods_{$rule.id}"
                                              value="{$rule.goods}" class="toggle"></td>
                    <td align="center"><input type="hidden" name="rule[{$rule.id}][shipping]" id="shipping_{$rule.id}"
                                              value="{$rule.shipping}" class="toggle"></td>
                    <td nowrap="nowrap"><input type="text" name="rule[{$rule.id}][tax_percent]" class="textbox number"
                                               style="text-align: right;" value="{$rule.tax_percent}"> %
                    </td>
                    <td align="center"><a href="{$VAL_SELF}&delete_rule={$rule.id}" class="delete"
        ...
        <fieldset>
            ...
            <div id="country-region">
                <div><label for="country-list">{$LANG.address.country}</label><span><select name="addrule[country_id]"
                                                                                            id="country-list"
                                                                                            class="textbox no-custom-zone"
                                                                                            title="{$LANG.common.regions_all}">
               {foreach from=$COUNTRIES item=country}<option value="{$country.numcode}" {if $country.numcode==
                                                             $CONFIG.store_country}selected="selected" {/if}>{$country.name}</option>
                            {/foreach}
               </select></span>
                </div>
                <div><label for="state-list">{$LANG.address.state}</label><span><input name="addrule[county_id]"
                                                                                       type="text" id="state-list"
                                                                                       class="textbox"
                                                                                       value="{$VAL_TAX_STATE}"></span>
                </div>
            </div>
            <div><label for="rule-taxrate">{$LANG.settings.tax_rate}</label><span><input name="addrule[tax_percent]"
                                                                                         id="rule-taxrate" type="text"
