...
<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
    ...
    <div id="general" class="tab_content">
        ...
        <fieldset>
            <legend>{$LANG.catalogue.title_import_csv}</legend>
            <div><label for="import_source">{$LANG.catalogue.import_source} ({$LANG.common.max}:
                    {$UPLOAD_LIMIT})</label><span><input type="file" name="source" id="import_source"></span></div>
            <div><label for="import_format">{$LANG.catalogue.import_format}</label><span><select name="format"
                                                                                                 id="import_format">
		<option value="">{$LANG.catalogue.unknown_format}</option>
	  </select></span></div>
            <div><label for="opt_delimiter">{$LANG.catalogue.delimiter}</label><span><select name="delimiter"
                                                                                             id="opt_delimiter">
		<option value=",">,</option>
		<option value=";">;</option>
    ...
    <div id="revert" class="tab_content">
        <fieldset>
            <legend>{$LANG.catalogue.title_import_delete_previous}</legend>
            <table>
    ...
    <div id="general" class="tab_content">
        ...
        {if isset($EXAMPLES)}
        <fieldset>
            <legend>{$LANG.catalogue.title_data_example}</legend>
        ...
        <fieldset>
            ...
            <div><label for="opt_truncate">{$LANG.catalogue.import_replace_existing}</label><span><input type="checkbox"
                                                                                                         name="option[truncate]"
                                                                                                         id="opt_truncate"
                                                                                                         value="1"></span>
            </div>
        </fieldset>
        ...
        <fieldset>
            <legend>{$LANG.catalogue.title_import_options}</legend>
            <div><label for="opt_headers">{$LANG.catalogue.import_csv_headers}</label><span><input type="checkbox"
                                                                                                   name="option[headers]"
                                                                                                   id="opt_headers"
                                                                                                   value="1"></span>
            </div>
