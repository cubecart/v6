...
<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
        ...
        <table>
            ...
            <tr>
                ...
                <td width="30" align="center">
                    <a href="{$content.translate}" title="{$LANG.translate.trans_add}"><i class="fa fa-plus-circle"
    ...
    <div id="email_templates" class="tab_content">
        ...
        <fieldset>
            ...
            <table width="70%">
                ...
                <tr>
                    <td width="10"><input type="hidden" name="template_default[{$template.template_id}]"
                                          id="template_default_{$template.template_id}"
                                          value="{$template.template_default}" class="toggle unique"></td>
                    <td><a href="{$template.edit}">{$template.title}</a></td>
                    <td width="10"><a href="{$template.clone}"><i class="fa fa-files-o"
                                                                  title="{$LANG.common.clone}"></i></a></td>
                    <td width="10"><a href="{$template.edit}" title="{$LANG.common.edit}"><i
                                class="fa fa-pencil-square-o" title="{$LANG.common.edit}"></i></a></td>
                    <td width="10"><a href="{$template.delete}" class="delete"
    ...
    <div id="email_import" class="tab_content">
        ...
        <fieldset>
            <legend>{$LANG.common.import}</legend>
            <p>{$LANG.email.help_email_import}</p>
            <div>
                <select name="import">
                    <option value="">{$LANG.form.please_select}</option>
                    {foreach from=$EMAIL_IMPORT item=import}
                    <option value="{$import.file}">{$import.code}</option>
                    {/foreach}
                </select>
            </div>
        </fieldset>
        ...
        <fieldset>
            <legend>{$LANG.common.export}</legend>
    ...
    <div id="general" class="tab_content">
        ...
        {if $LANGUAGES}
        <fieldset>
            <div><label for="content_subject">{$LANG.common.subject}</label><span><input type="text"
                                                                                         name="content[subject]"
                                                                                         id="content_subject"
                                                                                         value="{$CONTENT.subject}"
                                                                                         class="textbox"></span></div>
            <div><label for="content_language">{$LANG.common.language}</label><span><select name="content[language]"
                                                                                            id="content_language"
                                                                                            class="textbox">
    ...
    <div id="general" class="tab_content">
        ...
        <fieldset>
            <div><label for="template_desc">{$LANG.email.template_name}</label><span><input type="text"
                                                                                            name="template[title]"
                                                                                            id="template_desc"
    ...
    <div id="email_html" class="tab_content">
        <h3>{$LANG.email.title_content_html}</h3>
        <textarea name="template[content_html]" id="template_html"
                  class="textbox fck fck-full">{$TEMPLATE.content_html}</textarea>
        <h3>{$LANG.email.title_macros}</h3>
        <table>
            <thead>
            <tr>
    ...
    <div class="form_control">
        ...
        <input type="submit" value="{$LANG.common.save}">{if isset($DISPLAY_DELETE_LINK)} <a href="{$LINK_DELETE}"
                                                                                             class="delete"
                                                                                             title="{$LANG.notification.confirm_delete}">{$LANG.common.delete}</a>{/if}
    </div>
