...
<div id="dashboard" class="tab_content">
    ...
    <div class="dashboard_content">
            ...
            <tr>
                <td valign="top" nowrap="nowrap" width="25%">
                    ...
                    <ul>
                        ...
                        {foreachelse}
                        <li>
                            <a href="?_g=reports&report[date][from]={$QUICK_TASKS.today}&report[date][to]={$QUICK_TASKS.today}">{$LANG.dashboard.task_orders_view_day}</a>
                        </li>
                        <li>
                            <a href="?_g=reports&report[date][from]={$QUICK_TASKS.this_weeks}&report[date][to]={$QUICK_TASKS.today}">{$LANG.dashboard.task_orders_view_week}</a>
                        </li>
                ...
                <td valign="top" nowrap="nowrap" width="25%">
                    ...
                    <div><a href="?_g=orders&action=edit&order_id={$order.cart_order_id}">{$order.cart_order_id}</a> -
        ...
        <div class="extension">
            <h4><span>{$extension.price}</span><a href="{$extension.url}" title="{$extension.name}" target="_blank">{$extension.name|truncate:42:"&hellip;":true}</a>
            </h4>
            <a href="{$extension.url}" target="_blank"><img src="{$extension.image}" alt="{$extension.name}"
                                                            width="150"></a>
        </div>
        {/foreach}
...
<div id="orders" class="tab_content">
    ...
    <div>
        <table>
            ...
            <tr>
                <td><a href="?_g=orders&action=edit&order_id={$order.cart_order_id}&source=dashboard">{$order.cart_order_id}</a>
                </td>
                <td>
                ...
                <td>
                    <a href="?_g=customers&action=edit&customer_id={$order.customer_id}">{if empty($order.first_name) &&
                ...
                <td>{$order.total}</td>
                <td>
                    <a href="?_g=orders&action=edit&order_id={$order.cart_order_id}&source=dashboard"
                       title="{$LANG.common.edit}"><i class="fa fa-pencil-square-o" title="{$LANG.common.edit}"></i></a>
                    <a href="?_g=orders&delete={$order.cart_order_id}&source=dashboard" class="delete"
                       title="{$LANG.notification.confirm_delete}"><i class="fa fa-trash"
                                                                      title="{$LANG.common.delete}"></i></a>
                    {if isset($order.notes)}
                    <a href="?_g=orders&action=edit&order_id={$order.cart_order_id}&source=dashboard#order_notes"
                       title="{foreach $order.notes as $note}{$note.time} {$note.content}{" \r\n"}{/foreach}"><img
                        src="{$SKIN_VARS.admin_folder}/skins/{$SKIN_VARS.skin_folder}/images/note.png"
                        alt="{$LANG.common.notes}"></a>
                    {/if}
                </td>
            </tr>
            {/foreach}
...
<div id="product_reviews" class="tab_content">
    ...
    <form action="?_g=products&node=reviews&origin=dashboard" method="post" enctype="multipart/form-data">
        ...
        <div class="note">
         <span class="actions">
         <input type="hidden" class="toggle" name="approve[{$review.id}]" id="approve_{$review.id}"
                value="{$review.approved}">
         <a href="{$review.edit}" class="edit" title="{$LANG.common.edit}"><i class="fa fa-pencil-square-o"
         ...
         <a href="{$review.delete}" class="delete" title="{$LANG.notification.confirm_delete}"><i class="fa fa-trash"
            ...
            {/section}
                ...
                <a href="?_g=products&product_id={$review.product_id}&action=edit">{$review.product.name}</a> &raquo;
                {$review.date} :: {$review.name} &lt;<a href="mailto:{$review.email}">{$review.email}</a>&gt;
                {$review.ip_address}
            </div>
        </div>
        {/foreach}
...
<div id="stock_warnings" class="tab_content">
    ...
    <table width="70%">
        ...
        <tr>
            <td><a href="?_g=products&action=edit&product_id={$warn.product_id}{if $warn.M_use_stock==1}#Options{/if}">{$warn.name}</a>
            </td>
            <td align="center" width="65" nowrap="nowrap">{if
                $warn.M_use_stock==1}{$warn.M_stock_level}{else}{$warn.I_stock_level}{/if}{if $warn.cached_name}
                - {$warn.cached_name}
                {/if}
            </td>
            <td width="30">
                <a href="?_g=products&action=edit&product_id={$warn.product_id}{if $warn.M_use_stock==1}#Options{/if}"
                   class="edit"><i class="fa fa-pencil-square-o" title="{$LANG.common.edit}"></i></a><a
                    href="?_g=products&delete={$warn.product_id}&dashboard=1" class="delete"
                    title="{$LANG.notification.confirm_delete}"><i class="fa fa-trash"
                                                                   title="{$LANG.common.delete}"></i></a>
            </td>
        </tr>
