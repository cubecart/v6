...
<div id="rebuild" class="tab_content">
    ...
    <form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
        <fieldset>
            <legend>{$LANG.maintain.title_rebuild_catalogue}</legend>
            <div><label for="prodViews">{$LANG.maintain.reset_views}</label><span><input type="checkbox" id="prodViews"
                                                                                         name="prodViews"
                                                                                         value="1"></span></div>
        </fieldset>
        <fieldset>
            <legend>{$LANG.settings.title_seo_urls}</legend>
            <div><label for="truncate_seo_custom">{$LANG.maintain.truncate_seo_custom}</label><span><input
                        type="checkbox" id="truncate_seo_custom" name="truncate_seo_custom" value="1"></span></div>
            <div><label for="truncate_seo_auto">{$LANG.maintain.truncate_seo_auto}</label><span><input type="checkbox"
                                                                                                       id="truncate_seo_auto"
                                                                                                       name="truncate_seo_auto"
                                                                                                       value="1"></span>
            </div>
            <div class="clear important"><strong>{$LANG.maintain.truncate_seo_warning}</strong></div>
        </fieldset>
        <fieldset>
            <legend>{$LANG.maintain.title_rebuild_cache}</legend>
            <div><label for="clearCache">{$LANG.maintain.cache_clear}</label><span><input type="checkbox"
                                                                                          id="clearCache"
                                                                                          name="clearCache"
                                                                                          value="1"></span></div>
            <div><label for="clearSQLCache">{$LANG.maintain.cache_sql}</label><span><input type="checkbox"
                                                                                           id="clearSQLCache"
                                                                                           name="clearSQLCache"
                                                                                           value="1"></span></div>
            <div><label for="clearLangCache">{$LANG.maintain.cache_language}</label><span><input type="checkbox"
                                                                                                 id="clearLangCache"
                                                                                                 name="clearLangCache"
                                                                                                 value="1"></span></div>
            <div><label for="clearImageCache">{$LANG.maintain.cache_image}</label><span><input type="checkbox"
                                                                                               id="clearImageCache"
        ...
        <fieldset>
            <legend>{$LANG.maintain.title_rebuild_logs}</legend>
            <div><label for="clearLogs">{$LANG.maintain.logs_admin}</label><span><input type="checkbox" id="clearLogs"
                                                                                        name="clearLogs"
                                                                                        value="1"></span></div>
            <div><label for="emptyErrorLogs">{$LANG.maintain.logs_error}</label><span><input type="checkbox"
                                                                                             id="emptyErrorLogs"
                                                                                             name="emptyErrorLogs"
                                                                                             value="1"></span></div>
            <div><label for="emptyEmailLogs">{$LANG.maintain.logs_email}</label><span><input type="checkbox"
                                                                                             id="emptyEmailLogs"
                                                                                             name="emptyEmailLogs"
                                                                                             value="1"></span></div>
            <div><label for="emptyRequestLogs">{$LANG.maintain.logs_request}</label><span><input type="checkbox"
                                                                                                 id="emptyRequestLogs"
                                                                                                 name="emptyRequestLogs"
                                                                                                 value="1"></span></div>
            <div><label for="emptyTransLogs">{$LANG.maintain.logs_transaction}</label><span><input type="checkbox"
                                                                                                   id="emptyTransLogs"
                                                                                                   name="emptyTransLogs"
                                                                                                   value="1"></span>
            </div>
            <div><label for="clearSearch">{$LANG.maintain.clear_search}</label><span><input type="checkbox"
                                                                                            id="clearSearch"
                                                                                            name="clearSearch"
                                                                                            value="1"></span></div>
        </fieldset>
        <fieldset>
            <legend>{$LANG.maintain.title_rebuild_misc}</legend>
            <div><label for="sitemap">{$LANG.maintain.sitemap}</label><span><input type="checkbox" id="sitemap"
                                                                                   name="sitemap" value="1"></span>
            </div>
        </fieldset>
        <div>
...
<div id="backup" class="tab_content">
    ...
    <form action="?_g=maintenance&node=index&files_backup=1#backup" method="post">
        ...
        <fieldset>
            <legend>{$LANG.maintain.backup_options}</legend>
            <div>
        ...
        <div>
            <input type="submit" name="backup" class="delete" title="{$LANG.notification.confirm_continue}"
                   value="{$LANG.maintain.tab_backup}">
            <input type="hidden" name="token" value="{$SESSION_TOKEN}">
        </div>
    </form>
    <br>
    ...
    <fieldset>
        <legend>{$LANG.common.downloads}</legend>
        {if $EXISTING_BACKUPS}
        {foreach from=$EXISTING_BACKUPS item=backup}
        <div>
            <label for="{$backup.filename}" class="wide"> <a href="{$backup.download_link}">{$backup.filename}</a> -
                {$backup.size}</label>
            <span class="actions">
	    {if $backup.restore_link}
	    <a href="{$backup.restore_link}" class="delete" title="{$backup.warning}"><i class="fa fa-refresh"
                                                                                     titl="{$LANG.common.restore}"></i></a>
	    {/if}
	    <a href="{$backup.download_link}"><i class="fa fa-download" title="{$LANG.common.download}"></i></a>
	    <a href="{$backup.delete_link}" class="delete" title="{$LANG.notification.confirm_delete}"><i
...
<div id="upgrade" class="tab_content">
    ...
    <form action="?_g=maintenance&upgrade={$LATEST_VERSION}#upgrade" method="post">
        <div>
            <input type="submit" name="backup" class="submit_confirm" title="{$LANG.notification.confirm_continue}"
                   value="{$UPGRADE_NOW}">
            <input type="hidden" name="token" value="{$SESSION_TOKEN}">
    ...
    <form action="{$VAL_SELF}#database" method="post" enctype="multipart/form-data">
        <fieldset>
            ...
            <table width="650">
                ...
                <tr>
                    <td><input type="checkbox" id="{$table.Name}" name="tablename[]" value="{$table.Name}"
                               class="table"></td>
                ...
                <tfoot>
                <tr>
                    <td><img src="{$SKIN_VARS.admin_folder}/skins/{$SKIN_VARS.skin_folder}/images/select_all.gif"
                             alt=""></td>
