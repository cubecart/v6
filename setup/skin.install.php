	...
	<div class="install-grid">
	  ...
	  <div class="install-grid-col">
		...
		<fieldset>
		  ...
		  <div><label for="form-dbpassword" class="help" title="{$LANG.setup.help_dbuserpass}">{$LANG.setup.db_password}</label><span class="password-wrap"><input type="password" autocomplete="off" name="global[dbpassword]" id="form-dbpassword"{if isset($FORM.global.dbpassword)} value="{$FORM.global.dbpassword}"{/if} class="textbox" /><button type="button" class="password-toggle" data-target="form-dbpassword" data-text-show="{$LANG.common.show}" data-text-hide="{$LANG.common.hide}" tabindex="-1">{$LANG.common.show}</button></span></div>
