...
<div id="frame">
        ...
        <div id="language">
            <form action="index.php" method="post" enctype="multipart/form-data">
                <select name="language" id="language-select" class="textbox">
                    ...
                    <option value="{$lang.code}" {$lang.selected}>{$lang.title}</option>
                    {/foreach}
                </select>
    ...
    <div id="error">
        ...
        <ul>
            {foreach from=$GUI_MESSAGE.errors item=error}
            <li>{$error}</li>
            {/foreach}
        </ul>
    </div>
    ...
    <form action="index.php" method="post" enctype="multipart/form-data">
        <div id="content">
            ...
            <div><input type="checkbox" id="licence_agree" name="licence" value="1"/> <label for="licence_agree">{$LANG.setup.licence_agree}</label>
            </div>
            {/if}
            ...
            <div>
                <span class="result">{if $file.status}<span class="pass">{$LANG.common.writable}</span>{else}<span
                        class="fail">{$LANG.common.read_only}</span>{/if}</span>
                {$file.name}
            </div>
            {/foreach}
            ...
            <fieldset>
                <div><label for="form-dbhhost" class="help" rel="" title="{$LANG.setup.help_dbhostname}">{$LANG.setup.db_host}</label><span><input
                            type="text" name="global[dbhost]" id="form-dbhost" value="{$FORM.global.dbhost}"
                            class="textbox required"/></span></div>
                <div><label for="form-dbusername" class="help" rel="" title="{$LANG.setup.help_dbusername}">{$LANG.account.username}</label><span><input
                            type="text" name="global[dbusername]" id="form-dbusername" value="{$FORM.global.dbusername}"
                            class="textbox required"/></span></div>
                <div><label for="form-dbpassword" class="help" rel="" title="{$LANG.setup.help_dbuserpass}">{$LANG.account.password}</label><span><input
                            type="password" autocomplete="off" name="global[dbpassword]" id="form-dbpassword"
                            value="{$FORM.global.dbpassword}" class="textbox"/></span></div>
                <div><label for="form-dbpassconf" class="help" rel="" title="{$LANG.setup.help_dbconfirmpass}">{$LANG.account.password_confirm}</label><span><input
                            type="password" autocomplete="off" name="global[dbpassconf]" rel="form-dbpassword"
                            id="form-dbpassconf" value="{$FORM.global.dbpassconf}" class="textbox confirm"/></span>
                </div>
                <div><label for="form-dbdatabase" class="help" rel="" title="{$LANG.setup.help_dbname}">{$LANG.setup.db_name}</label><span><input
                            type="text" name="global[dbdatabase]" id="form-dbdatabase" value="{$FORM.global.dbdatabase}"
                            class="textbox required"/></span></div>
                <div><label for="form-dbprefix" class="help" rel="" title="{$LANG.setup.help_dbprefix}">{$LANG.setup.db_prefix}</label><span><input
                            type="text" name="global[dbprefix]" id="form-dbprefix" value="{$FORM.global.dbprefix}"
		  ...
		  <select name="config[default_currency]" id="form-currency" class="textbox required">
            ...
            <fieldset>
                <div><label for="form-username" rel="">{$LANG.account.username}</label><span><input type="text"
                                                                                                    name="admin[username]"
                                                                                                    id="form-username"
                                                                                                    value="{$FORM.admin.username}"
                                                                                                    class="textbox required"/></span>
                </div>
                <div><label for="form-password" rel="">{$LANG.account.password}</label><span><input type="password"
                                                                                                    name="admin[password]"
                                                                                                    id="form-password"
                                                                                                    value="{$FORM.admin.password}"
                                                                                                    class="textbox required"/></span>
                </div>
                <div><label for="form-passconf" rel="">{$LANG.account.password_confirm}</label><span><input
                            type="password" name="admin[passconf]" id="form-passconf" rel="form-password"
                            value="{$FORM.admin.passconf}" class="textbox required confirm"/></span></div>
                <div><label for="form-realname" rel="">{$LANG.user.name_full}</label><span><input type="text"
                                                                                                  name="admin[name]"
                                                                                                  id="form-realname"
                                                                                                  value="{$FORM.admin.name}"
                                                                                                  class="textbox required"/></span>
                </div>
                <div><label for="form-email" rel="">{$LANG.common.email}</label><span><input type="text"
                                                                                             name="admin[email]"
                                                                                             id="form-email"
            ...
            <fieldset>
                <div><label for="form-drop" class="help" title="{$LANG.setup.install_drop_tables_explained}">{$LANG.setup.install_drop_tables}</label><span><input
                            type="checkbox" name="drop" id="form-drop" value="1"/> {$LANG.setup.install_drop_tables_explained}</span>
                </div>
            </fieldset>
            ...
            <div>{$LANG_UPGRADE_FROM_TO}<br/>
                ...
                <br/>{$LANG.setup.upgrade_click_continue}
            </div>
            <input type="hidden" name="progress" value="0"/>
            {/if}
            ...
            <div id="toolbar">
                {if isset($CONTROLLER.continue)}<span class="continue"><input type="submit" name="proceed"
                                                                              value="{$LANG.common.continue}"/></span>{/if}
                {if isset($CONTROLLER.retry)}<span class="continue"><input type="submit" name="retry"
                                                                           value="{$LANG.setup.button_retry}"/></span>{/if}
                {if isset($CONTROLLER.restart)}<span class="cancel"><input type="submit" name="cancel"
                                                                           value="{$LANG.setup.button_restart}"
                                                                           class="cancel"/></span>{/if}
            </div>
        </div>
    </form>
</div>
