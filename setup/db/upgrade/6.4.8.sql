...
UPDATE `CubeCart_email_content` SET `content_html` = CONCAT(`content_html`,'{if !empty($DATA.ship_tracking)}<p>Track your order:<br>{$DATA.ship_tracking}</p>{/if}') WHERE `content_type` = 'cart.order_complete'; #EOQ
UPDATE `CubeCart_email_content` SET `content_text` = CONCAT(`content_text`,'{if !empty($DATA.ship_tracking)}\r\n\r\nTrack your order:\r\n{$DATA.ship_tracking}{/if}') WHERE `content_type` = 'cart.order_complete'; #EOQ