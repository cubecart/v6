...
<table width="100%" cellpadding="3" cellspacing="10" border="0">
  ...
  <tr>
	<td width="140">Card Type</td>              
	<td>
	  <select name="cardType">
		<option value="1">Visa</option>
		<option value="2">MasterCard</option>
		{if $AMEX}<option value="8">American Express</option>{/if}
  ...
  <tr>
	...
	<td>
	  <input type="text" name="cvc2" value="" size="5" maxlength="{$MAX_CVV2}" style="text-align: center" class="textbox_small" />
  ...
  <tr>
	...
	<td>
	  <select name="startMonth">
		{foreach from=$CARD.start.months item=month}<option value="{$month.value}" {$month.selected}>{$month.display}</option>{/foreach}
	  </select>
	  /
	  <select name="startYear">
		{foreach from=$CARD.start.years item=year}<option value="{$year.value}" {$year.selected}>{$year.display}</option>{/foreach}
	  </select>
	</td>
  </tr>
  <tr>
	<td width="140">{$LANG.gateway.card_issue_no}</td>
	<td><input type="text" name="issue" value="" size="3" maxlength="3" style="text-align: center" class="textbox_small" /></td>
  </tr>
</table>
...
<table width="100%" cellpadding="3" cellspacing="10" border="0">			
  ...
  <tr>
	...
	<td><input type="text" name="addr2" value="{$CUSTOMER.add2}" size="50" /></td>
  </tr>
  <tr>
