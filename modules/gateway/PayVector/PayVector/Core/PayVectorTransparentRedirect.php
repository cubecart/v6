...
class PayVectorTransparentRedirect extends PayVectorBase {
    ...
    public function get3DSPostAuthenticationFields() {
        ...
        if ($this -> boIncludeFormTags) {
            $return = '<form name="ProcessForm" id="ProcessForm" method="POST" action="https://mms.payvector.net/Pages/PublicPages/TransparentRedirect.aspx" >';
        }
        $return .= '
    ...
    private function get3DSecureAuthenticationRequiredFromPostVariables() {
        ...
        try {
            ...
            if ($this -> boErrorActive && $this -> boDebugMode && $this -> szDebugEmail != NULL) {
                mail($this -> szDebugEmail, "FUNC: PayVectorHosted:getTransactionCompleteResultFromPostVariables()", "ERROR: \n" . print_r($this -> szErrorMessage, 1));
            }
        } catch (Exception $e) {
            $this -> boErrorActive = true;
            $this -> szErrorMessage = $e -> getMessage();
    ...
    private function IsReady() {
        ...
        if ($this -> boErrorActive && $this -> boDebugMode && $this -> szDebugEmail != NULL) {
            mail($this -> szDebugEmail, "FUNC: PayVectorTransparentRedirect:IsReady()", "ERROR: \n" . print_r($this -> szErrorMessage, 1));
        }
        return !$this -> boErrorActive;
    }
