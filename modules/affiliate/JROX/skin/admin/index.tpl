<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
  <div id="JROX" class="tab_content">
	<h3><a href="http://jam.jrox.com" target="_blank" title="{$LANG.jrox.module_title}">{$TITLE}</a></h3>
	<fieldset><legend>{$LANG.module.config_settings}</legend>
	  ...
	  <div><label for="module_url">{$LANG.common.url}</label><span><input type="text" id="module_url" name="module[URL]" value="{$MODULE.URL}" class="textbox" /></span></div>
	</fieldset>
  </div>
