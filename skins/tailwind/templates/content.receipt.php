  ...
  <div class="mx-auto max-w-2xl pt-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
    <div class="space-y-2 px-4 sm:flex sm:items-baseline sm:justify-between sm:space-y-0 sm:px-0">
      <div class="flex sm:items-baseline sm:space-x-4">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{$LANG.customer.order_count_single} #{$SUM.{$CONFIG.oid_col}|default:$SUM.order_id}</h1>
        <a href="#" class="hidden text-sm font-medium text-indigo-600 hover:text-indigo-500 sm:block">
          View invoice
          <span aria-hidden="true"> &rarr;</span>
        </a>
      </div>
      <p class="text-sm text-gray-600">{$LANG.basket.order_date}: <span class="font-medium text-gray-900">{$SUM.order_date_formatted}</span></p>
            ...
            <div class="sm:flex lg:col-span-7">
              ...
              <div class="mt-6 sm:ml-6 sm:mt-0">
                <h3 class="text-base font-medium text-gray-900">
                  <a href="#">{$item.name}{if !empty($item.product_code)} ({$item.product_code}){/if}</a>
                </h3>
                {if !empty($item.options)}
                <ul>
                {foreach from=$item.options item=option}<li>{$option}</li>{/foreach}
                </ul>
                {/if}
