...
<div class="show-for-small-only">
    ...
    <div class="panel" id="basket_item_{$hash}">
        <div class="row">
            ...
            <div class="small-7 columns text-right">
                ...
                <ul class="no-bullet">
                    ...
                    <li><strong>{if
                            empty($option.option_description)}{$option.option_name}{else}{$option.option_description}{/if}</strong>:
                        {$option.value_name|truncate:45:"&hellip;":true}{if !empty($option.price_display)}
                        ({$option.price_display}){/if}
                    </li>
                    {/foreach}
                </ul>
                {/if}
        ...
        <div class="row">
            <div class="small-6 columns">
                ...
                <a href="#" class="quan subtract" rel="{$hash}">
                    <svg class="icon">
                ...
                <a href="#" class="quan add" rel="{$hash}">
                    <svg class="icon">
        ...
        <div class="row hide" id="quick_update_{$hash}">
            <div class="small-offset-3 small-9 columns">
                <button type="submit" name="update" class="button secondary tiny marg-top"
                        value="{$LANG.basket.basket_update}">{$LANG.common.update}
                </button>
            </div>
        </div>
    </div>
    ...
    <table class="expand">
        ...
        <tr>
            <td width="10%" nowrap="nowrap">{$LANG.basket.shipping}
                ...
                <div id="getEstimateSmall" class="hide panel callout">
                    <h4>
                        <svg class="icon right" id="getEstimateClose" onclick="$('#getEstimateSmall').slideUp();">
                            <use xlink:href="#icon-times"></use>
                        </svg>
                        {$LANG.basket.specify_shipping}
                    </h4>
                    <div>
                        <label for="estimate_country_small" class="hide-for-small-only">{$LANG.address.country}</label>
                        <select name="estimate[country]" id="estimate_country_small"
                                class="nosubmit country-list field_small_only" rel="estimate_state_small">
                    ...
                    <div>
                        ...
                        <input type="text" name="estimate[state]" id="estimate_state_small" value="{$ESTIMATES.state}"
                               class="field_small_only" placeholder="{$LANG.address.state}">
                    </div>
                    <div>
                        <label for="estimate_postcode_small"
                               class="hide-for-small-only">{$LANG.address.postcode}</label>
                        <input type="text" class="field_small_only" value="{$ESTIMATES.postcode}"
                               placeholder="{$LANG.address.postcode}" id="estimate_postcode_small"
                               name="estimate[postcode]">
                    </div>
                    <div>
                        <input type="submit" name="get-estimate" class="button expand field_small_only"
                               value="{$LANG.basket.fetch_shipping_rates}">
                    </div>
        ...
        <tr>
            <td colspan="2">
                {$coupon.voucher} <a href="{$VAL_SELF}&remove_code={$coupon.remove_code}" title="{$LANG.common.remove}">
                    <svg class="icon remove-coupon">
                        <use xlink:href="#icon-times"></use>
                    </svg>
                </a>
            </td>
