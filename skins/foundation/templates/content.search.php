...
<form method="post" action="?_a=category" id="advanced_search_form" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="small-12 columns">
            <ul class="small-block-grid-5">
                ...
                <li><input type="checkbox" value="{$manufacturer.id}" id="manufacturer_{$manufacturer.id}"
                           name="search[manufacturer][]" {$manufacturer.selected}><label
                        for="manufacturer_{$manufacturer.id}">{$manufacturer.name}</label></li>
                {/foreach}
            </ul>
        </div>
    </div>
    ...
    <div class="row">
        <div class="small-8 columns"><input type="checkbox" name="search[inStock]" id="in_stock" value="1"><label
                for="in_stock">{$LANG.search.in_stock}</label></div>
    </div>
    {/if}
    <div class="row">
        <div class="small-8 columns"><input type="checkbox" name="search[featured]" id="featured_only" value="1"><label
                for="featured_only">{$LANG.search.featured_only}</label></div>
    </div>
    <div class="row">
        <div class="small-8 columns clearfix">
            <input type="submit" class="button" value="{$LANG.common.search}">
