...
<div class="box-basket-content">
    ...
    <div class="pad basket-detail">
        ...
        <ul class="no-bullet">
            ...
            <li class="clearfix">
                <div class="left">&hellip;</div>
            </li>
            {break}
            {/if}
            <li class="clearfix">
                <div class="left"><a href="{$item.link}" title="{$item.name}">{$item.quantity} &times;
                        {$item.name|truncate:25:"&hellip;"}</a></div>
        ...
        <div><a href="{$STORE_URL}/index.php?_a=checkout" class="button success expand nomarg">{if $CONFIG.ssl == 1}
                <svg class="icon">
                    <use xlink:href="#icon-lock"></use>
                </svg>
                {$LANG.basket.basket_secure_checkout}{else}{$LANG.basket.basket_checkout}{/if}</a></div>
        {if !$IS_USER}
        <div class="thinpad-top"><a href="{$STORE_URL}/index.php?_a=basket" class="button expand nomarg">{$LANG.basket.view_basket}</a>
        </div>
        {/if}
        {else}
