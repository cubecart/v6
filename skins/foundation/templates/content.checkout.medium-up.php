...
<div class="show-for-medium-up">
    <table class="expand">
        ...
        <tr>
            <td class="text-center"><a href="{$STORE_URL}/index.php?_a=basket&remove-item={$hash}">
            ...
            <td>
                <input name="quan[{$hash}]" type="text" value="{$item.quantity}" maxlength="3" class="quantity checkout"
                       {$QUAN_READ_ONLY}>
            </td>
        ...
        <tr>
            ...
            <td>{$LANG.basket.shipping}
                ...
                <div id="getEstimate" class="hide panel callout">
                    <h4>
                        <svg class="icon right" id="getEstimateClose" onclick="$('#getEstimate').slideUp();">
                            <use xlink:href="#icon-times"></use>
                        </svg>
                        {$LANG.basket.specify_shipping}
                    </h4>
                    <label for="estimate_country">{$LANG.address.country}</label>
                    <select name="estimate[country]" id="estimate_country" class="nosubmit country-list"
                            rel="estimate_state">
                    ...
                    <input type="text" name="estimate[state]" id="estimate_state" value="{$ESTIMATES.state}"
                           placeholder="{$LANG.address.state}">
                    <label for="estimate_postcode">{$LANG.address.postcode}</label>
                    <input type="text" value="{$ESTIMATES.postcode}" id="estimate_postcode"
                           placeholder="{$LANG.address.postcode}" name="estimate[postcode]">
                    <input type="submit" name="get-estimate" class="button expand"
        ...
        <tr>
            <td colspan="4"></td>
            <td>{$coupon.voucher} <a href="{$VAL_SELF}&remove_code={$coupon.remove_code}" title="{$LANG.common.remove}">
