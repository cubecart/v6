...
<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data" class="autosubmit" id="checkout_form">
        ...
        <div class="row">
            <div class="small-12 columns">
                <ul class="no-bullet center" id="gateway_error">
                    ...
                    <li>
                        <input name="gateway" type="radio" value="{$gateway.folder}" id="{$gateway.folder}" required
                               {$gateway.checked} rel="gateway_error"><label for="{$gateway.folder}">{$gateway.description}</label>
                        {if !empty($gateway.help)}
                        <a href="{$gateway.help}" class="info" title="{$LANG.common.information}">
                            <svg class="icon">
    ...
    {/if}
    <div class="clearfix">
        <div class="show-for-medium-up"><a href="{$STORE_URL}/index.php?_a=basket&empty-basket=true"
                                           class="button alert left">
                <svg class="icon">
                    <use xlink:href="#icon-trash-o"></use>
                </svg>
                {$LANG.basket.basket_empty}</a></div>
        <div class="show-for-medium-up">
            <button type="submit" name="update" class="button secondary left" value="{$LANG.basket.basket_update}">
                <svg class="icon">
                    <use xlink:href="#icon-refresh"></use>
                </svg>
                {$LANG.basket.basket_update}
            </button>
        </div>
        ...
        <button type="submit" name="proceed" id="checkout_proceed" class="button right">{$CHECKOUT_BUTTON}
            <svg class="icon">
...
<div class="show-for-medium-up">
    ...
    <ul class="small-block-grid-5 no-bullet">
        ...
        <li>
            <a href="{$product.url}" title="{$product.name}"><img src="{$product.img_src}" class="th"
                                                                  alt="{$product.name}"></a>
            <br>
