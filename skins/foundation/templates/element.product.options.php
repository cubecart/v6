   ...
   {foreach from=$OPTIONS item=option}
      ...
      <div class="row">
         <div class="small-12 columns">
            ...
            <div><input type="radio" name="productOptions[{$option.option_id}]" id="rad_option_{$value.assign_id}" value="{$value.assign_id}" class="nomarg{if $value.absolute_price == '1'} absolute{/if}"{if empty($_POST) && !empty($value.option_default)} checked="checked"{/if}{if !$CTRL_HIDE_PRICES} data-price="{$value.decimal_price}" data-price-original="{$value.decimal_price_original}"{/if}{if $smarty.foreach.options.first} rel="error_option_{$option.option_id}" {if $option.required}required{/if}{/if} data-image="{$value.image}">
      ...
      <div class="row">
         <div class="small-12 columns">
            ...
            <input type="text" name="productOptions[{$option.option_id}][{$option.assign_id}]" id="option_{$option.option_id}"{if $option.absolute_price == '1'} class="absolute"{/if}{if !$CTRL_HIDE_PRICES} data-price="{$option.decimal_price}" data-price-original="{$option.decimal_price_original}"{/if} {if $option.required}required{/if}>
            {elseif $option.type == Catalogue::OPTION_TEXTAREA}
            <textarea name="productOptions[{$option.option_id}][{$option.assign_id}]" id="option_{$option.option_id}"{if $option.absolute_price == '1'} class="absolute"{/if}{if !$CTRL_HIDE_PRICES} data-price="{$option.decimal_price}" data-price-original="{$option.decimal_price_original}"{/if} {if $option.required}required{/if}></textarea>
            {/if}
         </div>
      </div>
