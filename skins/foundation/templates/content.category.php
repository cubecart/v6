...
<ul class="medium-block-grid-6 text-center{if $CONFIG.catalogue_expand_tree ==1} show-for-medium-up{else} small-block-grid-3{/if}"
    data-equalizer>
...
<div class="row">
            ...
            <div class="row">
                ...
                <div class="small-9 medium-5 columns left">
                    <select name="sort" id="product_sort">
                        ...
                        <option value="{$sort.field}|{$sort.order}" {$sort.selected}>{$sort.name} ({$sort.direction})
                        </option>
                        {/foreach}
                    </select>
    ...
    <div class="medium-4 columns show-for-medium-up">
        <dl class="sub-nav right" id="layout_toggle">
            <dd><a href="#" class="grid_view">
                    <svg class="icon">
                        <use xlink:href="#icon-th-large"></use>
                    </svg>
                </a></dd>
            <dd class="active"><a href="#" class="list_view">
                    <svg class="icon">
...
<div id="ccScroll">
        ...
        <li>
            <form action="{$VAL_SELF}" method="post" class="panel add_to_basket">
                <div class="row product_list_view">
                    ...
                    <div class="small-3 columns">
                        <h3>
                            {if $product.ctrl_sale}<span class="old_price">{$product.price}</span> <span
                                class="sale_price">{$product.sale_price}</span>
                            {else}
                            {$product.price}
                            {/if}
                        </h3>
                        ...
                        <div class="row collapse">
                            <div class="small-12 columns">
                                <input type="submit" value="{$LANG.common.unavailable}"
                                       class="button small disabled expand marg-top" disabled>
                            </div>
                        </div>
                        ...
                        <div class="row collapse">
                            ...
                            <div class="small-8 columns">
                                <button type="submit" value="{$LANG.catalogue.add_to_basket}"
                                        class="button small postfix">{$LANG.catalogue.add_to_basket}
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="row collapse">
                            <div class="small-12 columns">
                                <input type="submit" value="{$LANG.catalogue.out_of_stock_short}" disabled
                                       class="button disabled expand small">
                            </div>
                        </div>
                        {/if}
                    </div>
                ...
                <div class="product_grid_view hide">
                    <div data-equalizer-watch>
                        <div class="text-center">
                            <a href="{$product.url}" title="{$product.name}"><img class="th" src="{$product.thumbnail}"
                                                                                  alt="{$product.name}"></a>
                        </div>
                        <h3><a href="{$product.url}" title="{$product.name}">{$product.name|truncate:38:"&hellip;"}</a>
                        </h3>
                        {if $product.review_score}
                        <div class="rating">
                            <div>
                    ...
                    *}
                    ...
                    <div class="row collapse marg-top">
                        ...
                        <div class="small-9 columns ">
                            <button type="submit" value="{$LANG.catalogue.add_to_basket}" class="button small postfix">
                    ...
                    <div class="row collapse marg-top">
                        <div class="small-12 columns">
                            <input type="submit" value="{$LANG.catalogue.out_of_stock_short}"
            ...
            <dl>
                <dd>
                    <select class="url_select">
                        ...
                        <option value="{$page_split.url}" {if $page_split.selected} selected{
                        /if}>{$page_split.amount}</option>
                        {/foreach}
                    </select>
                </dd>
            </dl>
    ...
    *}
    ...
    <a href="{$current}{http_build_query($params)}{$anchor}" data-next-page="{$params[$var_name]}"
       class="button tiny expand" id="ccScroll-next">{$LANG.common.more}
        <svg class="icon">
