 ...
 *
 * ⚠ PORTED VERBATIM from the reference skin, and deliberately so.
 *
 * This emits the Product / Offer / AggregateRating / BreadcrumbList JSON-LD that
 * Google reads for rich results. It contains no presentation, only data, so
 * there is nothing to restyle — and any well-meant edit risks silently
 * invalidating a store's structured data, which fails quietly and costs search
 * visibility weeks later.
 *
 * It is also entirely skin-agnostic: verified no $SKIN_FOLDER, no image paths,
 * no framework classes.
 *
 * If it needs changing, that is a CORE change to make in the reference skin
 * first, then port here — not a place to diverge.
 *}
