...
<div class="mx-auto max-w-md">
   ...
   <form action="{$VAL_SELF}" method="post" id="password_recovery" data-cc-validate class="cc-card mt-6 space-y-4 p-6">
      <div>
         ...
         <input type="password" name="password[password]" id="password" minlength="8" maxlength="64" autocomplete="new-password" required>
      </div>
      <div>
         <label for="passconf" class="cc-label">{$LANG.user.password_confirm}</label>
         <input type="password" name="password[passconf]" id="passconf" minlength="8" maxlength="64" data-match="#password" autocomplete="new-password" required>
      </div>
