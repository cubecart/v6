...
<div class="mx-auto max-w-md">
   ...
   <form action="{$VAL_SELF}" method="post" id="password_recovery" class="cc-card mt-6 space-y-4 p-6">
      <div>
         <label for="password" class="cc-label">{$LANG.account.password}</label>
         <input type="password" name="password[password]" id="password" maxlength="64" autocomplete="new-password" required>
      </div>
      <div>
         <label for="passconf" class="cc-label">{$LANG.user.password_confirm}</label>
         <input type="password" name="password[passconf]" id="passconf" maxlength="64" autocomplete="new-password" required>
      </div>
