jQuery(document).ready(function () {
    ...
    jQuery.debug = function (message) {
        if (window.console) {
            console.debug('CubeCart: ' + message);
        } else {
            alert(message);
        }
    };
    ...
    if (typeof gui_message_json != 'undefined' && typeof gui_message_json == 'object') {
        for (var id in gui_message_json) {
            $('#' + id).addClass('required-error').val('');
        }
    }
...
function updateStriping(element) {
    ...
    $('.list,.reorder-list').find('>div,tbody>tr').hover(function () {
