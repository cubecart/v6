...
class User {
	...
	public function formatAddress($address = array(), $user_defined = true, $estimate = false) {
		...
		if  ($user_defined 
			...
			&& $_GET['_a']!=='addressbook'
			&& (
					(empty($address['state']) && !empty($address['country']) && $GLOBALS['db']->select('CubeCart_geo_zone',false, array('country_id' => $country_id))) 
					|| 
					($GLOBALS['db']->select('CubeCart_geo_zone',false, array($state_field => $address['state']))==false) && $GLOBALS['db']->select('CubeCart_geo_zone',false, array('country_id' => $country_id))
				)
			)
		{
