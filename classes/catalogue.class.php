...
class Catalogue {
	...
	private function _productGallery($product_id = false) {
		if (isset($product_id) && is_numeric($product_id)) {
			...
			// Look for images
			if (($gallery = $GLOBALS['db']->select('`'.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_image_index` AS `i` INNER JOIN `'.$GLOBALS['config']->get('config', 'dbprefix').'CubeCart_filemanager` AS `f` ON i.file_id = f.file_id', false, 'i.product_id = '.$product_id, array('main_img' => 'DESC')))) {
				$duplicates = array();
